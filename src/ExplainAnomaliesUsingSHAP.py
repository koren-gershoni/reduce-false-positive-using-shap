...
class ExplainAnomaliesUsingSHAP:
    ...
    def __init__(self, num_anomalies_to_explain=100, reconstruction_error_percent=0.8, shap_values_selection='constant', explanation="shap"):
        """
        Args:
    ...
    def get_top_anomaly_to_explain(self, x_explain):
        ...
        predictions = pd.DataFrame(predictions, columns=x_explain.columns)
        ...
        columns.extend([f"squared_error_{x}" for x in list(x_explain.columns)])
        items = []
    ...
    def get_highest_shap_values(self, shap_values_df):
        ...
        for i in range(shap_values_df.shape[0]):
            ...
            if self.shap_values_selection == 'mean':
                ...
                num_explaining_features = len(shap_values)
    ...
    def explain_unsupervised_data(self, x_train, x_explain, autoencoder=None, return_shap_values=False):
        ...
        x_explain = x_explain.iloc[0]
        df_err, total_mse = self.get_errors_df_per_record(x_explain)
        ...
        if not return_shap_values:
            return df_top_err.to_dict()
        shap_values_all_features = np.zeros((len(x_explain), len(x_explain)))
        #
        background_set = self.get_background_set(x_train, 200).values
        for i in range(num_of_features):
            self.counter = df_top_err.index[i]

            if self.explanation == "shap":
                explainer = DeepExplainer(self.autoencoder, torch.Tensor(background_set))
                shap_values = explainer.shap_values(torch.Tensor(x_explain).reshape(1, -1))
                shap_values = np.array(shap_values)[:, 0, self.counter]
                ...
                explainer = LimeTabularExplainer(background_set)
                shap_values = explainer.explain_instance(x_explain)
                print(5)
            else:
                raise ValueError("not implemented explanation method")

            shap_values_all_features[self.counter] = np.array(shap_values)
            # 
            ...
            # shap_dict[record_idx] = all_sets_explaining_features[record_idx]
        return df_top_err['err'].to_dict(), csr_matrix(shap_values_all_features)
        # return reconstruction_loss_dict